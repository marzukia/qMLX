...
qmlx supports reasoning models that show their thinking process before giving an answer. Models like Qwen3 and DeepSeek-R1 wrap their reasoning in `<think>...</think>` tags, and qmlx can parse these tags to separate the reasoning from the final response.
