...
We welcome contributions to qmlx!
...
> 2. **Performance tests** — use **4-bit**. ~80% of qmlx users run 4-bit on M-series machines, so perf numbers must come from 4-bit to represent real user experience.
...
Run `qmlx bench --help` for the full flag list. For multimodal (image /
...
```bash
qmlx bench qwen3.5-4b-4bit | tee results_m4.txt
```
