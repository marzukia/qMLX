...
"""/metrics surfaces the R15 #300 ``qmlx_kv_cache_dtype`` gauge."""
...
def test_gauge_emits_three_series_one_active():
    ...
    text = "\n".join(lines)
    assert "# TYPE qmlx_kv_cache_dtype gauge" in text
    assert 'qmlx_kv_cache_dtype{dtype="bf16"} 0' in text
    assert 'qmlx_kv_cache_dtype{dtype="int8"} 0' in text
    assert 'qmlx_kv_cache_dtype{dtype="int4"} 1' in text


def test_gauge_reflects_int8_when_engine_is_int8():
    cfg = _cfg_with_engine("int8")
    text = "\n".join(_render_kv_cache_dtype_gauge(cfg))
    assert 'qmlx_kv_cache_dtype{dtype="int4"} 0' in text
    assert 'qmlx_kv_cache_dtype{dtype="int8"} 1' in text
    assert 'qmlx_kv_cache_dtype{dtype="bf16"} 0' in text
...
def test_gauge_defaults_to_bf16_when_unknown():
    ...
    cfg = SimpleNamespace()
    text = "\n".join(_render_kv_cache_dtype_gauge(cfg))
    assert 'qmlx_kv_cache_dtype{dtype="bf16"} 1' in text
    assert 'qmlx_kv_cache_dtype{dtype="int4"} 0' in text
