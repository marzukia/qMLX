...
"""Tests for MCP config discovery + back-compat after the vllm-mlx → qmlx
rename.

The rename moved the config search dir and env var to ``qmlx`` /
``QMLX_MCP_CONFIG`` while keeping the pre-rename ``vllm-mlx`` /
...
def _clear_mcp_env(monkeypatch):
    ...
    monkeypatch.delenv("QMLX_MCP_CONFIG", raising=False)
...
def test_qmlx_config_dir_is_discovered(monkeypatch, tmp_path):
    monkeypatch.setenv("HOME", str(tmp_path))
    cfg = _write_cfg(tmp_path / ".config" / "qmlx" / "mcp.json")
...
def test_qmlx_dir_preferred_over_legacy(monkeypatch, tmp_path):
    monkeypatch.setenv("HOME", str(tmp_path))
    new = _write_cfg(tmp_path / ".config" / "qmlx" / "mcp.json")
...
def test_qmlx_env_var_honored(monkeypatch, tmp_path):
    cfg = _write_cfg(tmp_path / "explicit.json")
    monkeypatch.setenv("QMLX_MCP_CONFIG", str(cfg))
    assert mcp_config._find_config_file() == cfg
...
def test_new_env_var_wins_when_both_point_to_real_files(monkeypatch, tmp_path):
    ...
    monkeypatch.setenv("QMLX_MCP_CONFIG", str(new))
...
def test_stale_new_env_var_falls_through_to_working_legacy(monkeypatch, tmp_path):
    ...
    old = _write_cfg(tmp_path / "old.json")
    monkeypatch.setenv("QMLX_MCP_CONFIG", str(tmp_path / "does-not-exist.json"))
    monkeypatch.setenv("VLLM_MLX_MCP_CONFIG", str(old))
...
def test_directory_env_var_falls_through_to_working_legacy(monkeypatch, tmp_path):
    ...
    a_dir.mkdir()
    old = _write_cfg(tmp_path / "old.json")
    monkeypatch.setenv("QMLX_MCP_CONFIG", str(a_dir))
    monkeypatch.setenv("VLLM_MLX_MCP_CONFIG", str(old))
    assert mcp_config._find_config_file() == old
