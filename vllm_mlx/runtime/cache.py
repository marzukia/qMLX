...
def load_prefix_cache_from_disk() -> None:
    ...
    Now that the in-memory prefix cache is gone (disk-only reuse), this is
    a no-op beyond a log line: the disk checkpoint tier is discovered
    lazily by the restore-on-miss path, so there is nothing to preload.
    ``load_cache_from_disk`` is still called for engines that keep their
    own on-disk entry cache.
    """
    cfg = get_config()
