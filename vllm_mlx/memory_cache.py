...
"""
KV-cache helpers for qmlx.

The in-memory prefix cache that used to live here was removed in the
SSD-first refactor (issue #16): the disk checkpoint tier in
``vllm_mlx.runtime.disk_kv_checkpoint`` is now the only reuse layer. What
remains are the shared, stateless helpers the disk path still needs:

- tokens-bin (de)serialisation and safetensors header inspection
- cache trimming / offset helpers for hybrid recurrent-state layers
- KV quantisation / dequantisation and turboquant compression
- ``estimate_kv_cache_memory`` and ``MemoryCacheConfig``
