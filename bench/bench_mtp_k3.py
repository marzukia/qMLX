...
class KRunStats:
    ...
    def accept_rate(self) -> float:
        return self.total_accepted / self.total_attempts if self.total_attempts > 0 else 0.0
...
def run_mtp_benchmark(
    ...
    for k in range(1, max_k + 1):
        ...
        for prompt_idx, prompt in enumerate(prompts):
            ...
            accept_attempts = 0
            total_accepted = 0
            pos_accepts: dict[int, tuple[int, int]] = {}  # pos -> (attempts, accepted)
            
            def track_accept(token_id, logprob, is_draft):
                nonlocal accept_attempts, total_accepted
                if is_draft:
                    accept_attempts += 1
                    total_accepted += 1
                    
                    # Track position (draft position in chain)
                    # This is a simplification - in reality we'd track chain position
                    pos = 1  # For K=1 this is always pos 1
                    if k > 1:
                        # For K>1, we'd need to track chain position
                        # This requires modifications to the generator callback
                        pass
                    
                    if pos not in pos_accepts:
                        pos_accepts[pos] = (0, 0)
                    attempts, accepted = pos_accepts[pos]
                    pos_accepts[pos] = (attempts + 1, accepted + 1)
            ...
            stats.total_attempts += accept_attempts
            stats.total_accepted += total_accepted
            
            print(f"  Generated {tokens_generated} tokens, "
                  f"accept rate: {total_accepted}/{accept_attempts} = "
                  f"{100*total_accepted/accept_attempts if accept_attempts > 0 else 0:.1f}%")
    ...
    for k in sorted(all_stats.keys()):
        ...
        print(f"{k:<4} {stats.decode_tok_per_sec:<16.2f} {stats.accept_rate*100:<14.1f}% {speedup:<10.2f}x")
...
def main():
    ...
    parser.add_argument(
        ...
        type=int,
        default=3,
        help="Maximum K value to test (1, 2, or 3)"
    )
    parser.add_argument(
        "--max-tokens",
        type=int,
